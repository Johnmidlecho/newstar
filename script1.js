document.addEventListener("DOMContentLoaded", function() {
    ...
    function drawStars(starPositions, longIndex) {
        ...
        starKeys.forEach(key => {
            ...
            const number = currentPoint[`${key} number`];
            const gender = currentPoint[`${key} gender`];
            const text = currentPoint[`${key} text`];
            ...
            if (position) {
                ...
                if (imagePath) {
                    console.log(`${key} is person ${person} and number ${number}, showing image at:`, position);
    ...
    function createStar(x, y, isMis = false, starKey = null, longIndex = null, workSentence = null) {
        ...
        if (starKey === 'S1004') {
            ...
            Object.assign(centerStar.style, {
                fontSize: '10px',
                color: 'rgb(164, 239, 15)',
                textShadow: '0 0 20px rgb(172, 241, 24)',
...
function handlePersonImage(imagePath, position) {
    console.log('handlePersonImage called with position:', position);
    ...
    function showPersonImage(position) {
        ...
        Object.assign(personElement.style, {
            position: 'absolute',
            left: `${adjustedX-30}px`,
            top: `${adjustedY+300}px`,
            display: 'block',
            zIndex: '5',
            cursor: 'pointer',
            transform: 'translate(-50%, -50%)',
            transformOrigin: 'center center',
            width: `${personImageSize/1.2}px`,
            height: `${personImageSize/1.2}px`,
